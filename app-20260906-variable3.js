    ...
    <div class="card form-card">
      ...
      <form id="recurringForm">
        ...
        <div id="amountModeBox" class="amount-mode-box">
          <div class="field-title">Le montant est-il toujours le même ?</div>
          <div class="amount-choice-grid">
            ...
            <label class="amount-choice">
              ...
              <span><strong>Montants variables</strong><small>Un montant différent selon le mois</small></span>
            </label>
          </div>
        </div>
            ...
            ${MONTH_NAMES.map((n,i)=>{
              ...
              return `<div class="month-entry">
                <label class="month-chip">
                  <input class="month-check" type="checkbox" value="${m}" ${checked?"checked":""}>${n}
                </label>
                <input class="month-amount" data-month="${m}" type="number"
                       inputmode="decimal" step="0.01" min="0"
                       value="${val}" placeholder="Montant">
              </div>`;
            }).join("")}
          </div>
  ...
  if(recurringForm){
    ...
    const syncUI=()=>{
      ...
      document.querySelectorAll(".month-entry").forEach(entry=>{
        ...
        amount.style.display=(!isMonthly && amountMode==="variable" && check.checked)?"block":"none";
      });
