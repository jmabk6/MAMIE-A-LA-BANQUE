...
function recurringFormView(type, id=null){
  ...
  const variable=Object.keys(variableAmounts).length>0;
  return `
    <div class="card form-card">
      ...
      <form id="recurringForm">
        ...
        <label>Libellé<input id="recurringLabel" type="text" required value="${r?escapeHtml(r.label):""}" placeholder="${isRecette?"Pension retraite":"Charges appartement"}"></label>
        <label>Fréquence
        ...
        <div id="amountModeBox" class="amount-mode-box">
          <div class="field-title">Montant des échéances</div>
          <label class="radio-row"><input type="radio" name="amountMode" value="same" ${!variable?"checked":""}> Même montant à chaque échéance</label>
          <label class="radio-row"><input type="radio" name="amountMode" value="variable" ${variable?"checked":""}> Montant différent selon le mois</label>
        </div>
        ...
        <div id="monthsBox" class="months-box">
          ...
          <div class="month-grid variable-month-grid">
            ${MONTH_NAMES.map((n,i)=>{
              const m=i+1, checked=selected.includes(m), val=variableAmounts[String(m)] ?? "";
              return `<div class="month-entry">
                <label class="month-chip"><input class="month-check" type="checkbox" value="${m}" ${checked?"checked":""}>${n}</label>
                <input class="month-amount" data-month="${m}" type="number" inputmode="decimal" step="0.01" min="0" value="${val}" placeholder="Montant">
              </div>`;
            }).join("")}
          </div>
          <div class="meta">Coche les mois concernés. Si les montants varient, indique le montant sous chaque mois.</div>
        </div>
...
function bind(){
  ...
  if(recurringForm){
    const getAmountMode=()=>document.querySelector('input[name="amountMode"]:checked')?.value||"same";

    const collectTemp=()=>{
      const type=document.getElementById("recurringType").value;
      const frequency=document.getElementById("recurringFrequency").value;
      const months=[...document.querySelectorAll(".month-check:checked")].map(x=>Number(x.value));
      const variable=getAmountMode()==="variable";
      const amount=Number(document.getElementById("recurringAmount").value)||0;
      const variableAmounts={};
      if(variable) months.forEach(m=>{
        const el=document.querySelector(`.month-amount[data-month="${m}"]`);
        variableAmounts[String(m)]=Number(el?.value)||0;
      });
      return {type,frequency,months,amount,variableAmounts:variable?variableAmounts:{}};
    };

    const refreshProvision=()=>{
      const temp=collectTemp();
      const preview=document.getElementById("provisionPreview");
      const p=monthlyProvision(temp);
      preview.style.display=(temp.type==="prelevement" && temp.frequency!=="monthly")?"block":"none";
      preview.innerHTML=p?`Total annuel : <strong>${euro(annualRecurringTotal(temp))}</strong><br>À provisionner : <strong>${euro(p)}/mois</strong>`:"";
    };

    const syncRecurringUI=()=>{
      const variable=getAmountMode()==="variable";
      const frequency=document.getElementById("recurringFrequency").value;
      document.getElementById("sameAmountBox").style.display=variable?"none":"block";
      document.getElementById("monthsBox").style.display=frequency==="monthly"?"none":"block";
      document.querySelectorAll(".month-entry").forEach(entry=>{
        const check=entry.querySelector(".month-check");
        const amount=entry.querySelector(".month-amount");
        amount.style.display=(variable && check.checked)?"block":"none";
      });
      refreshProvision();
    };

    document.getElementById("recurringFrequency").addEventListener("change",syncRecurringUI);
    document.getElementById("recurringAmount").addEventListener("input",refreshProvision);
    document.querySelectorAll('input[name="amountMode"]').forEach(x=>x.addEventListener("change",syncRecurringUI));
    document.querySelectorAll(".month-check").forEach(x=>x.addEventListener("change",syncRecurringUI));
    ...
    recurringForm.onsubmit=e=>{
      ...
      if(temp.frequency!=="monthly" && temp.months.length===0){
        alert("Coche au moins un mois d’échéance."); return;
      }
      if(variable){
        for(const m of temp.months){
          const el=document.querySelector(`.month-amount[data-month="${m}"]`);
          if(el.value==="" || !Number.isFinite(Number(el.value)) || Number(el.value)<0){
            alert(`Indique un montant valide pour ${MONTH_NAMES[m-1]}.`); return;
          }
        }
      }else if(!Number.isFinite(Number(document.getElementById("recurringAmount").value))){
        alert("Indique un montant valide."); return;
      }
      ...
      const obj={
        ...
        months:temp.frequency==="monthly"?[]:temp.months,
        variableAmounts:variable?temp.variableAmounts:{}
      };
