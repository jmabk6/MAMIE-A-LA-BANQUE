...
const legacySeedBtn=
// Gestion globale des dépenses : fonctionne même après un re-render/recherche.
document.addEventListener("click", e=>{
  const row=e.target.closest && e.target.closest("[data-edit-tx]");
  if(!row) return;
  e.preventDefault();
...
// Neutralise le double-clic / double-tap qui provoque le zoom sur iPhone.
...
function renderTxList(list){
  ...
  return `<div class="card">${sortedTx(list).map(tx=>`
    <div class="tx ${tx.type==="depense"?"tx-editable":""}"
         ${tx.type==="depense" ? `data-edit-tx="${escapeHtml(String(tx.id))}" role="button" tabindex="0" aria-label="Modifier ${escapeHtml(tx.label)}"` : ""}>
      <div class="tx-main">
...
function addView(defaultType="depense", unknown=false, editId=null){
  ...
  if(editId!==null && !existing){
    setTimeout(()=>alert("Impossible de retrouver cette dépense. Recharge l’application puis réessaie."),0);
  }
  ...
  return `
    <form id="transactionForm" class="card form-card" data-edit-id="${existing ? escapeHtml(String(existing.id)) : ""}">
      ...
      <button class="secondary-btn transaction-cancel-btn" type="button" data-transaction-cancel="1" style="margin-top:10px">
        Annuler
      </button>
    </form>
  `;
